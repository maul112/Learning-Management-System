...
interface LessonMarkdownDisplayProps {
  content: string;
  className?: string;
}

export default function LessonMarkdownDisplay({
  content,
  className,
}: LessonMarkdownDisplayProps) {
  return (
    <div
      className={cn(
        'lesson-content prose dark:prose-invert max-w-none',
        className,
      )}
    >
      <ReactMarkdown
        ...
        components={{
          ...
          code({ node, inline, className, children, ...props }: any) {
            ...
            if (!inline && match) {
              return (
                <InteractiveCodeBlock code={codeContent} language={match[1]} />
              );
            }
          ...
          blockquote: ({ node, ...props }) => (
            <blockquote
              className="border-primary/30 bg-muted/20 my-4 rounded-r border-l-4 py-2 pl-4 italic"
              {...props}
            />
          ),
          ...
          th: ({ node, ...props }) => (
            <th
              className="px-3 py-2 text-left text-xs font-medium tracking-wider uppercase"
              {...props}
            />
          ),
          ...
          img: ({ node, ...props }) => (
            <img
              className="mx-auto my-4 h-auto max-w-full rounded-md"
              {...props}
            />
          ),
        }}
